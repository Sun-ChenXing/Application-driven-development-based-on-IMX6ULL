...
static int sr04_probe(struct platform_device *pdev)
{
	...
	err = request_irq(irq, sr04_handler, IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING, "scx_sr04_irq", NULL);

	device_create(sr04_class, NULL, MKDEV(major, 0), NULL, "scx_sr04"); 
...
static int __init sr04_init(void)
{
	...
	//注册设备
	major = register_chrdev(0, "sr04_scx", &sr04_ops);

	//创建类，自动分配设备结点信息
	sr04_class = class_create(THIS_MODULE, "sr04_class_scx");
	if (IS_ERR(sr04_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "sr04_scx");
