...
static int dht11_probe(struct platform_device *pdev)
{
	...
	device_create(dht11_class, NULL, MKDEV(major, 0), NULL, "scx_dht11");
...
static int __init dht11_init(void)
{
	...
	//注册设备
	major = register_chrdev(0, "scx_dht11", &dht11_ops);

	//创建类，自动分配设备结点信息
	dht11_class = class_create(THIS_MODULE, "scx_dht11_class");
	if (IS_ERR(dht11_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "scx_dht11");
