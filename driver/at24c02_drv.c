...
static long at24c02_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	err = copy_from_user(ker_buf, usr_buf, 2);	//第一次获取长度  包含地址和数据长度信息
	...
	switch (cmd)
	{
		case IOC_AT24C02_READ:
		{
			...
			// 1. 写操作的 I2C 消息，表示要向 AT24C02 发送一个地址
			msgs[0].addr  = at24c02_client->addr; //AT24C02 EEPROM 的 I2C 地址（通过 at24c02_client->addr 获取
			msgs[0].flags = 0; /* 写 */
			msgs[0].len   = 1; //发送 1 字节数据，即要读取的地址。
			msgs[0].buf   = &addr; //发送的地址数据存储在 addr 变量中。

			// 2. 读取操作的 I2C 消息，表示从 AT24C02 读取数据
			msgs[1].addr  = at24c02_client->addr; //同样是 AT24C02 的 I2C 地址
			msgs[1].flags = I2C_M_RD; /* 读 */
			msgs[1].len   = ker_buf[1];
			msgs[1].buf   = &ker_buf[2];
		
			// 3. 函数执行 I2C 传输操作。它会将 msgs 中定义的两个消息（一个写操作和一个读操作）通过 I2C 总线发送到 AT24C02
		...
		case IOC_AT24C02_WRITE:
		{
			...
			msgs[0].addr  = at24c02_client->addr;
			msgs[0].flags = 0; /* 写 */
			msgs[0].len   = size + 1;               // 数据长度 + 地址
			msgs[0].buf   = &ker_buf[1];            // 发送的数据（地址 + 数据）

			i2c_transfer(at24c02_client->adapter, msgs, 1);

			//在写入操作后，程序等待 20 毫秒，以确保 AT24C02 完成数据写入。AT24C02 通常在写操作后需要一些时间来完成数据存储
			mdelay(20);
...
static int at24c02_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//注册设备
	major = register_chrdev(0, "scx_at24c02", &at24c02_ops);

	//创建类，自动分配设备结点信息
	at24c02_class = class_create(THIS_MODULE, "scx_at24c02_class");
	if (IS_ERR(at24c02_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "scx_at24c02");
		return PTR_ERR(at24c02_class);
	}
	
	device_create(at24c02_class, NULL, MKDEV(major, 0), NULL, "scx_at24c02"); 
