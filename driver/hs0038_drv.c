...
static int hs0038_probe(struct platform_device *pdev)
{
	...
	device_create(hs0038_class, NULL, MKDEV(major, 0), NULL, "scx_hs0038"); 
...
static int __init hs0038_init(void)
{
	...
	//注册设备
	major = register_chrdev(0, "scx_hs0038", &hs0038_ops);

	//创建类，自动分配设备结点信息
	hs0038_class = class_create(THIS_MODULE, "scx_hs0038_class");
	if (IS_ERR(hs0038_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "scx_hs0038");
