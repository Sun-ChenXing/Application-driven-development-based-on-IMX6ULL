...
static int sg90_probe(struct platform_device *pdev)
{
    ...
    //pwm子系统
    // pwm_config(pwm_test, 0, 20000000);   /* 配置PWM：1.5ms，90度，周期：20000000ns = 20ms */
    pwm_set_polarity(pwm_test, PWM_POLARITY_NORMAL); /* 设置输出极性：占空比为高电平 */
    pwm_enable(pwm_test);    /* 使能PWM输出 */

	device_create(sg90_class, NULL, MKDEV(major, 0), NULL, "scx_sg90"); 
...
static int __init sg90_init(void)
{
	...
	//注册设备
	major = register_chrdev(0, "scx_sg90", &sg90_ops);

	//创建类，自动分配设备结点信息
	sg90_class = class_create(THIS_MODULE, "scx_sg90_class");
	if (IS_ERR(sg90_class)) {
		printk("%s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
		unregister_chrdev(major, "scx_sg90");
