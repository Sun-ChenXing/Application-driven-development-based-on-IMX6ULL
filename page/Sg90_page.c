...
uint8_t sg90_anglea_data = 0;

#define LABEL_DATA_CHILD_ID		1		//代表屏幕父对象的第2个子对象  角度数据显示文本
...
{ 
	...
	//角度数据显示文本	id_1
    lv_obj_t * Label_data2 = lv_label_create(sg90_page); // 创建一个标签对象
	lv_obj_set_style_text_font(Label_data2, ft_info.font, LV_STATE_DEFAULT); // 设置标签的字体
	lv_label_set_recolor(Label_data2, 1);  // 启用文本的颜色替换
	lv_label_set_text(Label_data2, "#519ABA 舵机当前角度：0°#"); // 设置标签文本
    lv_obj_align(Label_data2, LV_ALIGN_CENTER, 0, 10); // 将标签居中对齐，并设置位置

    //舵机控制滑块	id_2
    lv_obj_t* slider1 = lv_slider_create(sg90_page);  // 创建一个滑块控件
    lv_obj_align_to(slider1, Label_data2, LV_ALIGN_CENTER, 0, 30);  // 将滑块对齐到标签下方，y 偏移 30
    lv_slider_set_value(slider1, 0, LV_ANIM_OFF);  // 设置滑块的初始值为 0，并且关闭动画
    lv_obj_add_flag(slider1, LV_OBJ_FLAG_ADV_HITTEST);  // 启用高级点击测试
    lv_slider_set_range(slider1, 0, 180);  // 设置滑块的范围从 0 到 180

    //1.打开舵机设备
    Sg90_fd = open("/dev/scx_sg90", O_RDWR);
    if(Sg90_fd < 0)
    {
        perror("打开设备失败");
        return ;
    ...
    // //2.打开红外遥控器设备
    // Hs0038_fd = open("/dev/input/event3", O_RDWR);
    // if(Hs0038_fd < 0)
    // {
    //     perror("打开红外遥控器设备失败");
    //     return ;
    ...
    lv_obj_add_event_cb(slider1, my_btn_value_event_cb, LV_EVENT_VALUE_CHANGED, NULL);	//添加事件回调函数
}
