...
uint16_t file_umg_cnt;		//指示当前访问的图片是第几张
...
static void my_btn_up_event_cb(lv_event_t * e)
{
	...
	char file_name[50]; // 用于存储图片文件路径

	if(code_e == LV_EVENT_CLICKED)
	{
		if(file_umg_cnt < (file_name_num_max-2))	//如果还有图片
		{
			sprintf(file_name, "/home/img/%03d.jpg", ++file_name_num_id); // 格式化字符串，生成下一个图片的文件路径（例如：/home/img/001.jpg）
			FILE *file = fopen(file_name, "r"); // 尝试打开该文件
			while(1)		//开始循环遍历到第一个非空文件
			{
				if(file == NULL) 
				{
					// 文件未找到，递增文件编号并继续尝试下一个文件
					sprintf(file_name, "/home/img/%03d.jpg", ++file_name_num_id);
					file = fopen(file_name, "r"); // 重新打开下一个文件
				}
				else	//遍历到非空文件，退出
				{
					fclose(file);
				...
				// 如果文件编号达到最大值，表示文件命名格式不正确，显示错误信息
			...
			file_umg_cnt++;  // 增加图片计数
			sprintf(file_name, "S:/home/img/%03d.jpg", file_name_num_id); // 格式化文件路径，更新显示当前文件的路径
			lv_label_set_text_fmt(Label_name, "#519ABA 当前查看的文件:/home/img/%03d.jpg#", file_name_num_id); // 更新标签控件，显示当前图片的路径
    		lv_img_set_src(img, file_name); 	// 设置图像数据源
		}
		else
		{
			//如果已经到了最后一张 那么重新从第一张开始遍历
			...
			sprintf(file_name, "S:/home/img/%03d.jpg", file_name_num_id);
			lv_label_set_text_fmt(Label_name, "#519ABA 当前查看的文件:/home/img/%03d.jpg#", file_name_num_id);
			lv_img_set_src(img, file_name); 	// 设置图像数据源
		}
	}
}
...
{ 
	...
	lv_obj_set_size(Frame_obj, 640, 480);													//设置画面显示大小
	...
	// lv_label_set_text(Label_name, "#519ABA 当前查看的文件:/home/img/001.jpg#");
	lv_obj_align_to(Label_name, Frame_obj, LV_ALIGN_TOP_LEFT, 0, -50);

	char file_name[50];
	file_name_num_max = list_files("/home/img/");
