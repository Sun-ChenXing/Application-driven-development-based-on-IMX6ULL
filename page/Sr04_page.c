...
// #define FRAME_OBJ_CHILD_ID		1		//代表屏幕父对象的第2个子对象  测距画面显示的区域
#define LABEL_STATE_CHILD_ID	4		//代表屏幕父对象的第5个子对象  设备状态显示文本
#define SR04_SW_CHILD_ID		3		//代表屏幕父对象的第4个子对象  超声波测距的开关
#define LABEL_DATA_CHILD_ID		2		//代表屏幕父对象的第3个子对象  数据显示文本
#define Bar_CHILD_ID			5		//代表屏幕父对象的第6个子对象  距离值显示的滑条
...
static void my_btn_return_event_cb(lv_event_t * e)
{
	...
	if(code_e == LV_EVENT_CLICKED)
	{
		...
		if(lv_obj_has_state(Sr04_sw, LV_STATE_CHECKED))	//如果用户按下返回时仍然是打开的状态则清除摄像头数据
		{
...
{
	lv_obj_t * Label_data = lv_obj_get_child(Sr04_page, LABEL_DATA_CHILD_ID);//获取了显示距离数据的标签 
	lv_obj_t * Bar_Distance = lv_obj_get_child(Sr04_page, Bar_CHILD_ID);//显示距离值的进度条 
...
static void my_Sw_Sr04_event_cb(lv_event_t * e)
{
	...
	if(code_e == LV_EVENT_VALUE_CHANGED)
	{
		...
		{
			//1.打开超声波设备
			Sr04_fd = open("/dev/scx_sr04", O_RDWR);
			if(Sr04_fd < 0)
			{
				perror("打开设备失败");
...
{ 
	//创建组
	lv_group_t * g = lv_group_create(); // 创建一个新的输入设备组
	lv_group_set_default(g);								//添加到默认组
	lv_indev_set_group(mouse_indev, g);	//输入设备添加进组

	//创建返回按钮		id_0
    lv_obj_t * btn_return = lv_btn_create(Sr04_page); // 在页面 Sr04_page 上创建一个按钮
    lv_obj_set_size(btn_return, 100, 100); // 设置按钮的大小为 100x100 像素
	lv_obj_align(btn_return, LV_ALIGN_TOP_LEFT, 20, 20);  // 将按钮定位到页面的左上角，距离左边和上边 20 像素

	lv_obj_t * label_return = lv_label_create(btn_return); // 在按钮上创建一个标签
	FT_font_Set(28);  // 设置字体大小为 28
	lv_obj_set_style_text_font(label_return, ft_info.font, LV_STATE_DEFAULT); // 设置标签文本的字体，使用自定义的字体
	lv_label_set_text(label_return, "返回"); // 设置标签的文本为 "返回"
	lv_obj_align(label_return, LV_ALIGN_CENTER, 0, -4); // 将标签居中对齐，并在垂直方向上稍微向上移动 4 像素

	//创建测距画面显示的区域	id_1
	lv_obj_t * Frame_obj= lv_btn_create(Sr04_page); // 创建一个新的按钮对象，用作画面显示区域
	...
	//距离数据显示文本	id_2
	lv_obj_t * Label_data = lv_label_create(Sr04_page);  // 创建一个标签对象，用于显示距离数据
	lv_obj_set_style_text_font(Label_data, ft_info.font, LV_STATE_DEFAULT); // 设置标签的字体为自定义字体
	lv_label_set_recolor(Label_data, 1); // 启用标签文本的颜色功能
	lv_label_set_text(Label_data, "#519ABA 距离:NULL#");  // 设置标签文本，初始化显示 "距离:NULL"
	lv_obj_align_to(Label_data, Frame_obj, LV_ALIGN_TOP_LEFT, 0, 0);  // 将标签定位到 Frame_obj 的左上角，偏移量为 (0, 0)

	//创建开始超声波测距的开关	id_3
	lv_obj_t * Sr04_sw = lv_switch_create(Sr04_page);  // 创建一个开关组件
	lv_obj_set_size(Sr04_sw, 100, 60); // 设置开关的大小为 100x60 像素
	lv_obj_align_to(Sr04_sw, Frame_obj, LV_ALIGN_OUT_TOP_LEFT, 0, -10);  // 将开关放置在 Frame_obj 的顶部，偏移量 (0, -10)

	//设备状态显示文本	id_4
	lv_obj_t * Label_state = lv_label_create(Sr04_page);  // 创建一个标签对象，用于显示设备状态（例如，"停止测距"）
	lv_obj_set_style_text_font(Label_state, ft_info.font, LV_STATE_DEFAULT);
	lv_label_set_recolor(Label_state, 1); // 启用标签文本的颜色功能
	lv_label_set_text(Label_state, "#519ABA 停止测距#");  // 设置标签文本，初始化显示 "停止测距"
	lv_obj_align_to(Label_state, Sr04_sw, LV_ALIGN_CENTER, 160, 0); // 将标签定位到开关组件旁边，偏移量为 (160, 0)

	//距离值显示的滑条	id_5
	lv_obj_t * Bar_Distance = lv_bar_create(Sr04_page);  				// 创建一个条形图（滑条）组件，用于显示测距值
	lv_obj_set_size(Bar_Distance,300,20);                   			// 设置条形图的大小为 300x20 像素
	lv_obj_align_to(Bar_Distance, Label_data, LV_ALIGN_CENTER, 90, 40); // 将条形图对齐到 Label_data 标签的中心，并设置偏移量为 (90, 40)
	lv_bar_set_value(Bar_Distance,0,LV_ANIM_OFF);  // 设置条形图的初始值为 0，并关闭动画效果

	lv_obj_add_event_cb(btn_return, my_btn_return_event_cb, LV_EVENT_CLICKED, NULL);	//添加事件回调函数
	lv_obj_add_event_cb(Sr04_sw, my_Sw_Sr04_event_cb, LV_EVENT_VALUE_CHANGED, NULL);	//添加事件回调函数
}
