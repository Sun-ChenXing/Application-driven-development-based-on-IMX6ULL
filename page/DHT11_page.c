...
#define LABEL_STATE_CHILD_ID	5  //设备状态显示文本
#define DHT11_sw_CHILD_ID		4  //DHT11的开关
#define LABEL_DATA_CHILD_ID		2  //距离温度显示文本
#define LABEL_DATA2_CHILD_ID	3  //距离湿度显示文本
...
static void my_Sw_DHT11_event_cb(lv_event_t * e)
{
	...
	if(code_e == LV_EVENT_VALUE_CHANGED)
	{
		...
		{
			//1.打开超声波设备
			DHT11_fd = open("/dev/scx_dht11", O_RDWR);
			if(DHT11_fd < 0)
			{
				perror("打开设备失败");
...
{ 
	...
	lv_obj_add_event_cb(DHT11_sw, my_Sw_DHT11_event_cb, LV_EVENT_VALUE_CHANGED, NULL);  //添加事件回调函数
}
