...
static void my_btn_write_event_cb(lv_event_t * e)
{
    ...
    if(code_e == LV_EVENT_CLICKED)
	{
        ...
        At24_fd = open("/dev/scx_at24c02", O_RDWR);
        if(At24_fd < 0)
		{
            LV_LOG_ERROR("打开设备失败");
...
//动画可以动态更新目标对象的属性，如位置、透明度、大小等，同时允许你根据动画的当前值执行相应的操作
static void my_btn_indev_set_value(void *btn, int32_t value)
{
    //btn 是传递的动画目标对象，在这里是 BtnM_indev
    //value 是动画的当前值，在每个动画步骤中，value 会随着动画的进展而更新。
...
static void my_btn_addr_event_cb(lv_event_t * e)
{
	...
	if(code_e == LV_EVENT_CLICKED)
	{
        ...
        lv_btnmatrix_set_map(BtnM_indev, (const char **)btnMap2);//指向常量字符指针的指针
        ...
        lv_anim_t anim;         //创建动画
        lv_anim_init(&anim);
        lv_anim_set_exec_cb(&anim, my_btn_indev_set_value); //设置动画回调函数
        lv_anim_set_values(&anim, 0, 100);
        lv_anim_set_path_cb(&anim, lv_anim_path_linear);
        lv_anim_set_time(&anim, 300);
        lv_anim_set_var(&anim, BtnM_indev); // 设置动画的目标对象
	    lv_anim_start(&anim);
...
{ 
	...
	lv_obj_set_style_opa(Frame_obj, LV_OPA_100, LV_STATE_DEFAULT);							//设置画面背景透明度60%
    ...
    //创建存储画面	id_2
    lv_obj_t *BtnM_addr = lv_btnmatrix_create(Frame_obj);  // 创建按钮矩阵并将其附加到父对象 Frame_obj 中
    
    // 为按钮矩阵分配 49 个按钮的文本存储空间
    for(uint8_t i=0; i<49 ;i++)
    {
        btnMap[i] = malloc(sizeof(char) * 20);  // 为每个按钮的文本动态分配 20 字节的内存
        if(btnMap[i] == NULL)
        {
            printf("分配内存失败！\n");
            return ;
    ...
    char p[20]; // 用于格式化文本的临时字符数组
    for(uint8_t i=0, j=0; i<49 ;i++)
    {
        // 格式化按钮的文本标签，每个按钮显示 "0xXX" 形式的十六进制地址
        sprintf(p, "0x%02X\n#0078FF NULL", i-j);

        // 将格式化的文本复制到 btnMap[i] 中
        strcpy(btnMap[i], p);

        // 每 7 个按钮一行处理
        if((i+1)%7==0)  
        {
            if(i == 48) break; // 如果是最后一个按钮，则跳出循环
            strcpy(btnMap[i], "\n"); // 如果是行尾的按钮，设置文本为换行符

            // 格式化新的按钮标签并将其存储到 btnMap[i+1]
            sprintf(p, "0x%02X\n#0078FF NULL", i);
            strcpy(btnMap[i+1], p);

            // 如果是第 7 个按钮，跳过下一个按钮的索引
            if(i == 6) i++;
    ...
    lv_obj_set_size(BtnM_addr, 620, 440); // 设置按钮矩阵的大小
    lv_obj_set_style_border_opa(BtnM_addr, LV_OPA_0, LV_STATE_DEFAULT); // 设置按钮矩阵的边框透明度为 0（即无边框）
    lv_obj_set_style_bg_opa(BtnM_addr, LV_OPA_0, LV_STATE_DEFAULT); // 设置按钮矩阵的背景透明度为 0（即无背景）
    lv_btnmatrix_set_map(BtnM_addr, (const char **)btnMap); // 设置按钮矩阵的按钮标签映射，btnMap 包含了每个按钮的文本
    lv_btnmatrix_set_btn_ctrl_all(BtnM_addr, LV_BTNMATRIX_CTRL_RECOLOR); // 设置按钮矩阵的所有按钮都支持重新着色功能
    lv_obj_align(BtnM_addr, LV_ALIGN_CENTER, 0, 0); // 将按钮矩阵对齐到父容器的中心

    // LV_LOG_USER("%p", BtnM_addr);

    //写入按钮	id_3
	lv_obj_t * Btn_write = lv_btn_create(At24c02_page); // 创建按钮对象，附加到页面 At24c02_page
    lv_obj_set_size(Btn_write, 120, 60); // 设置按钮的大小为 120x60
	lv_obj_set_style_radius(Btn_write, 15, LV_STATE_DEFAULT); // 设置按钮的圆角半径为 15
	lv_obj_align_to(Btn_write, Frame_obj, LV_ALIGN_TOP_RIGHT, 160, 40); // 将按钮相对于 Frame_obj 对齐，偏移量为 (160, 40)
	lv_obj_t * Label_write = lv_label_create(Btn_write); // 创建一个标签对象，并将其附加到 Btn_write 按钮上
	lv_obj_set_style_text_font(Label_write, ft_info.font, LV_STATE_DEFAULT);  // 设置标签的字体
	lv_label_set_text(Label_write, "一键写入"); // 设置标签的文本为 "一键写入"
	lv_obj_align(Label_write, LV_ALIGN_CENTER, 0, -3); // 将标签对齐到按钮的中心，垂直偏移 -3

    //读取按钮	id_4
	lv_obj_t * Btn_read = lv_btn_create(At24c02_page); // 创建按钮对象，附加到页面 At24c02_page
    lv_obj_set_size(Btn_read, 120, 60); // 设置按钮的大小为 120x60
	lv_obj_set_style_radius(Btn_read, 15, LV_STATE_DEFAULT); // 设置按钮的圆角半径为 15
	lv_obj_align_to(Btn_read, Btn_write, LV_ALIGN_TOP_RIGHT, 20, 80);  // 将按钮相对于 Btn_write 对齐，偏移量为 (20, 80)
	lv_obj_t * Label_read = lv_label_create(Btn_read); // 创建标签对象，附加到 Btn_read 按钮上
	lv_obj_set_style_text_font(Label_read, ft_info.font, LV_STATE_DEFAULT); // 设置标签的字体
	lv_label_set_text(Label_read, "一键读取"); // 设置标签的文本为 "一键读取"
	lv_obj_align(Label_read, LV_ALIGN_CENTER, 0, -3); // 将标签对齐到按钮的中心，垂直偏移 -3

    //文本提示	id_5
    lv_obj_t * Label_Str = lv_label_create(At24c02_page); // 创建文本提示标签，附加到页面 At24c02_page
    lv_obj_set_style_text_font(Label_Str, ft_info.font, LV_STATE_DEFAULT); // 设置标签的字体
    lv_label_set_recolor(Label_Str, 1); // 使文本支持颜色变化
    lv_obj_align_to(Label_Str, Frame_obj, LV_ALIGN_OUT_TOP_MID, -40, -10); // 将标签对齐到 Frame_obj 的上方，中间位置
    lv_label_set_text(Label_Str, "#0000FF EEPROM区域#"); // 设置标签的文本为 "文本提示"
